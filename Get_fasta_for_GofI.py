...
Namelist=['WAI332','WAI320','WAI324','WAI326','WAI329','WAI55','WAI56','WAI147','WAI327','WAI322','WAI323','WAI321','WAI328']
#Import reference genome annotation
refgene={}
for seq_record in SeqIO.parse("/Users/meganm/Genomes/IPO323/Zym_jason_annotation/10688_gDNA.fasta", "fasta"):
...
x=2
for name in Namelist:
    for seq_record2 in SeqIO.parse("/Volumes/Solomon_Lab/Megan/Spades_assembly/RRes_anno_pres_absence/shortname_fastas/"+name+".50percent.short.out.fasta", "fasta"):
...
for line in refgene:
    fs=open("/Volumes/Solomon_Lab/Megan/Spades_assembly/RRes_anno_pres_absence/genomefastas/"+line+".fasta", 'w')
    IPOline=">IPO323-%s\n%s\n" % (line, refgene[line][1])
    ...
    WAI147line=">WAI147-%s\n%s\n" % (line, refgene[line][11][1])
    WAI327line=">WAI327-%s\n%s\n" % (line, refgene[line][12][1])
    WAI321line=">WAI321-%s\n%s\n" % (line, refgene[line][13][1])
    WAI328line=">WAI328-%s\n%s\n" % (line, refgene[line][14][1])
    fs.write(IPOline+WAI332line+WAI320line+WAI324line+WAI326line+WAI329line+WAI55line+WAI56line+WAI147line+WAI327line+WAI322line+WAI323line+WAI321line+WAI328line)
    fs.close()
...
for key in refgene:
    missing_genes=[]
    good_genes=[]
    for record in SeqIO.parse("/Volumes/Solomon_Lab/Megan/Spades_assembly/gdnablast/genomefastas/"+key+".fasta", "fasta"):
        if len(record.seq) < 10:
            missing_genes.append(record)
        else:
            good_genes.append(record)
        output1=open("/Volumes/Solomon_Lab/Megan/Spades_assembly/gdnablast/genomefastas/"+key+".fasta", "w")
...
for key in refgene:
    in_file ="/Volumes/Solomon_Lab/Megan/Spades_assembly/gdnablast/genomefastas/"+key+".fasta"
    out_file ="/Volumes/Solomon_Lab/Megan/Spades_assembly/gdnablast/genomealignments/"+key+"_align.fasta"
