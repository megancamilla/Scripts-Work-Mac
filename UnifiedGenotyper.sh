...
if [ "$1" == "" ]
	then
	    echo "-----------------------------------------------------------------"
	    echo "USAGE:"
	    echo "Enter all of the BAM files you would like to analyse on the command line"
	    echo "Tool used is GATK Unified Genotyper with ploidy set to 1"
	    echo "UnifiedGenotyper --> java -jar ~/bin/GATK/GenomeAnalysisTK.jar -T UnifiedGenotyper -R Reference.fasta -I $1 $2 $3 ... -o Ztritici.all.vcf";
	    echo "example usage:"
	    echo "GATK_Realigner.sh WAI321.reduced.bam WAI322.reduced.bam"
	    echo "-----------------------------------------------------------------"
	    exit 1
	else
	 echo "Computing variants for: $@"	 
	 java -Xmx90g -jar ~/bin/GATK/GenomeAnalysisTK.jar -T UnifiedGenotyper -R /data/Megan/IPO323/Mycosphaerella_graminicola.allmasked.fa -I WAI320.reduced.bam -I WAI321.reduced.bam -I WAI322.reduced.bam -I WAI323.reduced.bam -I WAI324.reduced.bam -I WAI326.reduced.bam -I WAI327.reduced.bam -I WAI328.reduced.bam -I WAI329.reduced.bam -o Ztritici.GATK.vcf -ploidy 1 -stand_call_conf 50 -glm BOTH
