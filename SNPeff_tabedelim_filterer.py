...
def main(inSNPeff=None, refFasta=None, SecretedList=None, outSNPeff=None, maxGeneSize=250, Cysteine=0.05):
...
if __name__== '__main__':
	...
	arg_parser = argparse.ArgumentParser(
		...
		and extracts from an SNPeff annotated one per line file (SNPS) that occur in genes in the given list.\
		Will also take into account length and cysteine content of the reference protein")
	arg_parser.add_argument("-i","--inSNPeff", default=None, required=True, help="Path to SnpEff VCF file to be filtered, required")
	...
	arg_parser.add_argument("-s","--SecretedList", default=None, required=True, help="List of Secreted protien names one column, if you don't want to filter by secreted feed in list of reference proteins, required")
	arg_parser.add_argument("-o","--outSNPeff", default=None, required=True, help="Path to output file, required")
	...
	inSNPeff=args.inSNPeff
	refFasta=args.refFasta
	SecretedList=args.SecretedList
	outSNPeff=args.outSNPeff
	maxGeneSize=args.maxGeneSize
	Cysteine=args.Cysteine
	
	main(inSNPeff, refFasta, SecretedList, outSNPeff, maxGeneSize, Cysteine)