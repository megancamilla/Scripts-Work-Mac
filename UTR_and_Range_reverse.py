...
for Line in InFile:
    ...
    if int(ElementList[4])==0:
        OutputStringFor = "-entry scaffold_%s -range %d-%d" % (ElementList[1], (int(ElementList[2])-1000), int(ElementList[2]))
        OutFileFor.write(OutputStringFor+"\n")
    elif int(ElementList[4])==1:
        OutputStringRev = "-entry scaffold_%s -range %d-%d" % (ElementList[1], int(ElementList[3]), (int(ElementList[3])+1000))
