...
InFileName = '/data/Megan/IPO323/Mgraminicolav2.FrozenGeneCatalog20080910.gff'
...
for line in InFile:
    ...
    strand = fields[6]
    dot2 = fields[7]
    if type == "start_codon" or "stop_codon":
        namesearch = '(name) "(.+)"'
        Result = re.search(namesearch, line)
        name = Result.group(2)
        newline = "%s\tJGI\t%s\t%s\t%s\t%s\t%s\t%s\tID=gene:%s" % (chromosome, type, positionstart, positionstop, dot1, strand, dot2, name) 
